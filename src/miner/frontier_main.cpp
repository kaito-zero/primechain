...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        // Ported from primechain3/perf work, minus the finalization-
        // proposer-routing half of that same commit -- that part depended
        // on requestFinalizationProposer()/endpointForValidator(), which
        // don't exist here because upstream reverted "Add deterministic
        // finalization proposer routing" (7db0bb0) outright; re-adding it
        // would mean resurrecting reverted validator-side behavior with
        // zero server-side support in this protocol2 checkout. This half
        // -- skip re-submitting a proof we already know is doomed until
        // the cooldown-holder changes -- is purely client-side and has no
        // such dependency.
